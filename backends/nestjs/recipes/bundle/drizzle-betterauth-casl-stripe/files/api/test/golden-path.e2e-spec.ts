...
import { membership, tenant } from '../src/db/schema';
...
const rawDb = drizzle(rawPool, { schema: { tenant, membership } });
...
describe('golden path', () => {
  ...
  it('RBAC: a member is rejected on update/delete but still allowed read/create', async () => {
    ...
    await rawDb.update(membership).set({ role: 'member' }).where(eq(membership.tenantId, me.body.tenant.id));
