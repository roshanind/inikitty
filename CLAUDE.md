...
The **generator engine, CLI, and base template are implemented** (the "foundation pass"). The golden-path bundle recipe (`recipes/bundle/prisma-betterauth-casl-stripe/`) is being built incrementally as a single evolving recipe, per §8.4's "one tested integration unit" principle — **auth + Prisma, multi-tenancy (Postgres RLS), and CASL RBAC enforcement are done**; **auth + Prisma and multi-tenancy are verified end-to-end against a real Postgres, CASL is verified by generation/typecheck/build only** (no `Projects` resource yet to exercise a real `@CheckPolicies()` role check against). **Stripe billing and the `Projects` example resource are not built yet**. There's also one optional category recipe, `auth-extra/jwt-plugin`. No `AGENTS.md` generation or README/ARCHITECTURE.md assembly yet. See `recipes/README.md` for what each recipe actually contains and a list of non-obvious gotchas hit while building it (Prisma 7's config changes, NestJS asset-copying for a custom Prisma output path, pnpm's strict `node_modules` resolution, Postgres RLS + a non-superuser DB role, the `packages/shared` pnpm workspace CASL uses) — read that before touching the recipe.

The bundle now generates a **pnpm workspace** (root `pnpm-workspace.yaml`, `packages/shared/`) so `api/` and `app/` can share CASL's action/subject vocabulary and `defineAbilityFor()` isomorphically — see the "Sharing code between api/ and app/" gotcha in `recipes/README.md`. `src/cli.ts` detects the workspace file and runs a single root `pnpm install` instead of two separate installs when it's present.
...
- Every tenant-scoped app table carries a `tenant_id`-equivalent column (`Membership.tenantId` is the first one; `Tenant` itself doesn't have one — it's the scope boundary, not scoped data).
- Postgres RLS enforces tenant scoping at the DB level as a backstop against application-code bugs — **implemented** for `membership`, verified live by connecting as `app_role` directly and confirming cross-tenant reads return zero rows, not an error and not other tenants' data.
- The ORM layer auto-injects the tenant filter via a Prisma Client Extension (`forTenant()`) — a developer should not be able to write a query that forgets to scope by tenant. Recipe-generated code must always go through `TenantContext`'s scoped client, never query tenant-scoped tables directly with the plain `PrismaService`.
- CASL wired via a `PoliciesGuard` + `@CheckPolicies()` decorator, registered globally alongside Better Auth's `AuthGuard` (`api/src/casl/`); three seeded roles (`owner`, `admin`, `member` — the `MembershipRole` enum exists and is seeded on signup). Ability definitions (`defineAbilityFor()`) live in the `packages/shared` workspace package, not `api/`, so the same rules are importable from `app/` too — tenant-aware by construction, since callers always pass the role the user holds *within the active tenant* (`TenantContext.getRole()`), never a global role. `Subjects` currently only covers `Tenant`/`Membership`/`Project`; extend it when Stripe billing adds real subjects rather than guessing their shape now. No controller uses `@CheckPolicies()` yet — that lands with the `Projects` resource.
- Global `ValidationPipe` with `whitelist: true, forbidNonWhitelisted: true` — never remove this.
- Per-resource DTO convention: `CreateXDto`, `UpdateXDto` (as `PartialType(CreateXDto)`), `XResponseDto` with `@Exclude()` on sensitive fields — never return an entity directly. Not yet formally established anywhere (the tenancy slice's one endpoint is read-only and hand-picks response fields); the `Projects` resource (future) is where this gets a real worked example.
