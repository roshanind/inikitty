...
After that, the job runs `pnpm test` (unit), `pnpm build`, `pnpm test:e2e` (the real black-box e2e suite), and `app/`'s `tsc -b` + `vite build` — the exact sequence verified by hand throughout this project's development, now automated. Verified locally by running both scripts + every downstream step directly (not just reading the YAML) before ever pushing it.
