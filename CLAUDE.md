...
**Phase 1 (the golden path) is functionally complete, and Phase 2's alternate-ORM bundle is now built too.** The generator engine, CLI, and base template are implemented; the golden-path bundle recipe (`recipes/bundle/prisma-betterauth-casl-stripe/`) wires auth (Better Auth), multi-tenancy (Postgres RLS), RBAC (CASL), billing (Stripe), Material UI, and one worked-example resource (`Projects`) end to end — plus the generated project's own test suite, `AGENTS.md`, `README.md`, and `ARCHITECTURE.md`, all real and assembled from what's actually wired, not boilerplate. A second bundle, `recipes/bundle/drizzle-betterauth-casl-stripe/`, implements the identical guarantees against Drizzle instead of Prisma — mutually exclusive with the Prisma bundle, reusing `packages/shared` and every FE file byte-for-byte (neither has an ORM dependency). There's also four optional category recipes: `auth-extra/jwt-plugin` (now `requiresAnyOf` either bundle, not hardcoded to Prisma's) and three AI-agent vendor shims (`ai-format/claude-code`, `ai-format/cursor`, `ai-format/copilot`), all thin pointers into `AGENTS.md`. A real CI job (`.github/workflows/ci.yml`) now runs the whole golden path — generate, install, Docker Postgres, migrations, unit + e2e tests, frontend build — on every push, matrixed over `scripts/list-bundles.ts`'s output, so it already covers both bundles automatically with zero workflow changes. What's left for the broader spec (`docs/product-scope.md` §12) is remaining Phase 2/3 scope: alternate UI-library recipes, payment-provider alternatives, a monorepo toggle, and subdomain-based tenant resolution.
...
| ORM | Prisma (default) or Drizzle — two mutually exclusive bundles, both built |
| Auth | Better Auth |
...
- **Bundle recipes** — integration-coupled choices that must be tested together as one unit (e.g. ORM + auth provider + tenancy wiring + RBAC). Two bundles now exist, both functionally complete: `prisma-betterauth-casl-stripe` and `drizzle-betterauth-casl-stripe` — see `recipes/README.md`. A second ORM meant authoring a second bundle, not making ORM and auth independently pluggable — that integration seam is where "loosely wired" bugs happen (confirmed again while building the Drizzle bundle — see its gotchas in `recipes/README.md`), so each stays a single tested unit rather than being split into interchangeable ORM/auth pieces.
...
The selection/resolution logic was written generally from day one specifically so a second bundle wouldn't need engine changes — confirmed true in practice: adding `drizzle-betterauth-casl-stripe` required zero changes to `apply.ts`/`inject.ts`/`packageJson.ts`, one small additive change to `resolve.ts` (`requiresAnyOf`, below), and the golden-path CI check picked it up automatically via `scripts/list-bundles.ts`'s matrix with zero workflow changes.
...
- `resolve.ts` — enforces exactly one bundle when any bundle recipes exist, `conflicts`/`requires`/`requiresAnyOf`, and returns bundle-first + alphabetical-by-category-then-id ordering. `requiresAnyOf` (AND `requires`'s OR counterpart — satisfied if *any* listed id is selected) was added for `jwt-plugin`, which needs *some* better-auth-based bundle but shouldn't hardcode a specific one now that two exist.
...
Engine unit tests use small fixture recipes in `tests/fixtures/` (not real product recipes) so they don't churn as real recipes are added — including two fixture bundles (`alpha`/`beta`/`gamma`) specifically to cover `requiresAnyOf` across more than one bundle. `tests/smoke/real-template.test.ts` separately exercises the actual `templates/base` + `recipes/` — one case with no bundle selected, one with the real `prisma-betterauth-casl-stripe` + `jwt-plugin` + all three `ai-format` shims selected, one with `drizzle-betterauth-casl-stripe` + `jwt-plugin` selected (file shape and injected content only, `runPostInstall: false` since that needs Docker/network). Full behavioral verification (does signup/login/session actually work against a real Postgres) has been done manually, thoroughly, more than once per bundle as each grew, and now also runs automatically in CI (see below) — see `recipes/README.md`.
...
### Drizzle bundle design decisions (`drizzle-betterauth-casl-stripe` — where it diverges from the Prisma bundle)

Everything in the sections above describes `prisma-betterauth-casl-stripe`; the Drizzle bundle implements identical guarantees (same RLS policies, same CASL rules, same DTO/RBAC/testing conventions, same `Projects` worked example, same FE files reused byte-for-byte) but with these genuine differences — see `recipes/README.md`'s Drizzle gotchas for the full list, including two real bugs only caught by live e2e testing:

- **No Prisma-Client-Extension equivalent exists in Drizzle.** `TenantContext.withTenant(fn)` (not `getPrisma()`) is the tenant-scoping entry point everywhere — it opens a real transaction, sets the Postgres session variable, then invokes a caller-supplied callback with the transaction handle, rather than returning an already-scoped client to query directly. `tenant.extension.ts`'s `withTenant()`/`withUser()` are plain higher-order functions, not `$extends`-based middleware.
- **Ids are Postgres `gen_random_uuid()`, not a JS-generated id.** `@paralleldrive/cuid2` was tried first (for id-shape parity with Prisma's `cuid()` defaults) but ships ESM-only, breaking Jest transitively through `schema.ts` — the exact class of bug already known for `@thallesp/nestjs-better-auth`. Dropping the dependency entirely (native `uuid` columns) was simpler than another `jestModuleNameMapper` workaround.
- **RLS policies cast the column to text, not the session setting to uuid** (`"tenantId"::text = current_setting(...)`, never `current_setting(...)::uuid`) — a real, live-verified bug: a custom Postgres GUC that's had a `SET LOCAL` value committed on a pooled connection reverts to `''` (empty string), not `NULL`, on a later transaction that never sets it — and `''::uuid` throws. Casting the column to text instead makes an unset/reverted setting just fail the equality check normally.
- **`better-auth/adapters/drizzle` is a thin re-export of the separate `@better-auth/drizzle-adapter` package** (confirmed by reading `better-auth`'s actual installed `dist/`, not its docs site, which showed an inconsistent import path) — pinned as a direct `api/` dependency for the same pnpm-strict-node_modules reason `zod` already is.
- **`enable-rls.sql` is applied directly via `docker compose exec ... psql`, not folded into drizzle-kit's own migration journal** (`drizzle/meta/_journal.json`) — that format is undocumented and version-sensitive, unlike Prisma's plain-timestamped-folder migrations which are trivial to fabricate one for.
- Both bundles' `postInstall.ts` and CI verification are otherwise structurally identical (build `packages/shared` → Docker Postgres → schema generation → migrate → RLS → unit tests → build → e2e → frontend build) — confirmed via the identical `scripts/golden-path-*.ts` scripts running unmodified against either bundle.
...
- Vendor-specific shims (`CLAUDE.md`, `.cursor/rules/`, Copilot instructions) are optional category recipes, never a default, and are always thin pointers into `AGENTS.md` — never duplicated content, to avoid drift. **All three are built**: `ai-format/claude-code` (`CLAUDE.md`, pointing at `./AGENTS.md`), `ai-format/cursor` (`.cursor/rules/agents.mdc`, frontmatter `alwaysApply: true`, body uses Cursor's `@AGENTS.md` file-reference syntax so Cursor actually pulls the real content into context rather than just linking to it), and `ai-format/copilot` (`.github/copilot-instructions.md`, pointing at `../AGENTS.md` — GitHub Copilot Chat automatically includes this file's content in every request in the repo). None has any real content of its own. Resolution order (bundle-first, then alphabetical by category-then-id) means all three sort as `claude-code`, `copilot`, `cursor` when multiple are selected together.
...
**CI (`.github/workflows/ci.yml`) is implemented** — three jobs: `generator` (typecheck/lint/test/build, fast, no Docker), `discover-bundles` (runs `scripts/list-bundles.ts`, which calls `discoverRecipes()` and filters for `category === 'bundle'`, to build a matrix), and `golden-path` (matrixed over that output). This was written when v1 had exactly one bundle specifically so a second would need zero workflow changes — now proven true: adding `drizzle-betterauth-casl-stripe` made the matrix pick it up automatically, no CI edits required. The `golden-path` job mirrors a real `npx create-inikitty` run exactly, via two small scripts rather than reimplementing generation/install/postInstall logic in YAML:
...
**Phase 2's Drizzle bundle is done too** — `drizzle-betterauth-casl-stripe`, functionally complete and live-verified with the identical rigor as the Prisma bundle (see the Drizzle design-decisions section above and `recipes/README.md`'s Drizzle gotchas), automatically covered by the same CI matrix.

Remaining Phase 2/3 (not started): Antd/shadcn-ui as alternate UI-library recipes; payment-provider alternatives; a monorepo toggle; subdomain-based tenant resolution. (The §8.3/§9.1 "generate → install → migrate → test" verification loop is done — see the Testing strategy section above and `.github/workflows/ci.yml`.)
