...
- **Testing generated projects**: not yet implemented as *automated* tests shipped inside the generated project — every generated project should eventually ship working unit tests (service layer, tenancy/RBAC mocked) and integration/e2e tests (via `supertest`) covering signup → login → create project → list (own tenant only) → cross-tenant access rejected → RBAC-restricted action rejected for `member`. That exact flow has now been verified manually, by hand, via curl against a real running instance (see the Projects gotchas in `recipes/README.md`) — the automated version of it is what's still missing.

## MVP phasing

Phase 1 is golden-path only: NestJS + Prisma + Postgres (RLS) + Better Auth + CASL + Stripe + Vite/React + shadcn/ui, single recipe, no CLI branching beyond project name. It's not "done" when the app merely runs — it requires the full generated test suite passing, `README.md`/`ARCHITECTURE.md`/`/api/docs` present and accurate, and `AGENTS.md` + `docs/adding-a-resource.md` present and reflecting real wiring (see `docs/product-scope.md` §12 for the full checklist). `docs/adding-a-resource.md` exists now (written against the real `Project` implementation, not speculatively); `README.md`/`ARCHITECTURE.md` assembly, `AGENTS.md` generation, and the generated project's own automated test suite are still outstanding — shadcn/ui (the one stack-table item with no recipe yet) is also still unbuilt, so Phase 1 isn't fully closed out even though the bundle itself is feature-complete.
