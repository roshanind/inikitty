...
The **generator engine, CLI, and base template are implemented** (the "foundation pass"). The golden-path bundle recipe (`recipes/bundle/prisma-betterauth-casl-stripe/`) is being built incrementally as a single evolving recipe, per §8.4's "one tested integration unit" principle — **auth + Prisma, multi-tenancy (Postgres RLS), CASL RBAC enforcement, and Stripe billing are done**; **auth + Prisma and multi-tenancy are verified end-to-end against a real Postgres; CASL and Stripe billing are verified by generation/typecheck/build (plus, for the Stripe webhook, a real signature verify/reject round-trip via Stripe's own test helper) rather than live HTTP requests against a running app** (no `Projects` resource yet to exercise a real `@CheckPolicies()` role check against, and no real Stripe account was used). **The `Projects` example resource is not built yet** — it's the next and last piece of this bundle. There's also one optional category recipe, `auth-extra/jwt-plugin`. No `AGENTS.md` generation or README/ARCHITECTURE.md assembly yet. See `recipes/README.md` for what each recipe actually contains and a list of non-obvious gotchas hit while building it (Prisma 7's config changes, NestJS asset-copying for a custom Prisma output path, pnpm's strict `node_modules` resolution, Postgres RLS + a non-superuser DB role, the `packages/shared` pnpm workspace CASL/Stripe use, `bodyParser: false` needing per-route JSON/raw restoration for billing) — read that before touching the recipe.

The bundle generates a **pnpm workspace** (root `pnpm-workspace.yaml`, `packages/shared/`) so `api/` and `app/` can share CASL's action/subject vocabulary and `defineAbilityFor()` isomorphically — see the CASL gotchas in `recipes/README.md`. `src/cli.ts` detects the workspace file and runs a single root `pnpm install` instead of two separate installs when it's present.

Billing lives in `api/src/billing/`: `POST /billing/checkout` and `POST /billing/portal` (both owner-only, gated via `@CheckPolicies()` against a `Subscription` CASL subject) create hosted Stripe Checkout/Customer-Portal sessions; a public `POST /billing/webhook` verifies Stripe's signature and syncs a `Subscription` table (real Prisma relation to `Tenant`, RLS-protected); `@RequiresActiveSubscription()` is a guard for gating premium routes once they exist. `main.ts` gained two new marker comments (top-level `imports`, and `middleware` right after `NestFactory.create()`) so this recipe can restore JSON body parsing everywhere except `/auth/*` and give `/billing/webhook` the raw body its signature check needs — see the billing gotchas in `recipes/README.md` for why `bodyParser: false` made this necessary.
...
- **Bundle recipes** — integration-coupled choices that must be tested together as one unit (e.g. ORM + auth provider + tenancy wiring + RBAC). v1 ships exactly one bundle: `prisma-betterauth-casl-stripe` (currently implementing its auth + Prisma slice and multi-tenancy — see `recipes/README.md`). A second ORM means authoring a second bundle (`drizzle-betterauth-casl-stripe`), not making ORM and auth independently pluggable — that integration seam is where "loosely wired" bugs happen, so it stays a single tested unit until proven safe to split.
...
- Every tenant-scoped app table carries a `tenant_id`-equivalent column (`Membership.tenantId` was the first; `Subscription.tenantId` is the second — `Tenant` itself doesn't have one, it's the scope boundary, not scoped data).
- Postgres RLS enforces tenant scoping at the DB level as a backstop against application-code bugs — **implemented** for `membership` (verified live by connecting as `app_role` directly and confirming cross-tenant reads return zero rows, not an error and not other tenants' data) and for `subscription` (verified by typecheck/generation only, not a live cross-tenant read check like membership got — see `recipes/README.md`'s billing gotchas for why its policy only needs one `USING` branch, unlike membership's two).
...
- CASL wired via a `PoliciesGuard` + `@CheckPolicies()` decorator, registered globally alongside Better Auth's `AuthGuard` (`api/src/casl/`); three seeded roles (`owner`, `admin`, `member` — the `MembershipRole` enum exists and is seeded on signup). Ability definitions (`defineAbilityFor()`) live in the `packages/shared` workspace package, not `api/`, so the same rules are importable from `app/` too — tenant-aware by construction, since callers always pass the role the user holds *within the active tenant* (`TenantContext.getRole()`), never a global role. `Subjects` now covers `Tenant`/`Membership`/`Project`/`Subscription`; the billing endpoints are the first real `@CheckPolicies()` usage (owner-only, via `owner`'s `manage all` wildcard — no explicit `Subscription` rule was added for `admin`/`member`, so they're denied by omission). No controller guards an actual business resource yet — that lands with `Projects`.
- Global `ValidationPipe` with `whitelist: true, forbidNonWhitelisted: true` — never remove this.
- Per-resource DTO convention: `CreateXDto`, `UpdateXDto` (as `PartialType(CreateXDto)`), `XResponseDto` with `@Exclude()` on sensitive fields — never return an entity directly. Not yet formally established anywhere (the tenancy slice's one endpoint is read-only and hand-picks response fields, and the billing slice's DTOs are single-field with no response class); the `Projects` resource (future) is where this gets a real worked example.
- The `Projects` resource (**not yet built**) is meant to be the canonical worked example (tenant-scoped, RBAC-guarded, DTO-validated, with FE pages) that all of the above conventions should match once it exists. Treat it as the reference implementation when writing or reviewing other recipe templates, once built.
