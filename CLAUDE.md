...
- CASL wired via a `PoliciesGuard` + `@CheckPolicies()` decorator; three seeded roles (`owner`, `admin`, `member` — the `MembershipRole` enum exists and is seeded on signup) — **not yet implemented**: the actual `PoliciesGuard`/ability definitions enforcing them. Abilities are meant to be tenant-aware (scoped to the user's membership/role within the active tenant, not global) once built.
